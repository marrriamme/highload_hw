...
**2. Данные профиля:**
- Средний размер: 50 КБ (текстовые данные)
...
**3. Учебный прогресс:**
- Средний размер: 100 КБ (структурированные данные о прогрессе)
- Общий объем: 49,2M × 100 КБ / 1 073 741 824 ≈ 4,92 ТБ

**4. Социальные связи:**
- Средний размер: 20 КБ (списки ID подписчиков/подписок)
...
3. Основной объем хранилища (99,9%) составляют аудиозаписи
...
- Это трафик на загрузку (upload). Скачивание аудио для проверки и прослушивания образцов может добавлять еще ~20% к этому объему. Итого: 183 ТБ * 1.2 ≈ 220 ТБ/день
...
1.  Пиковая нагрузка: активность пользователей распределена неравномерно. Пик нагрузки обычно в вечернее время. Для глобального сервиса с аудиторией по всему миру пик сглаживается, но не полностью. Возьмем консервативный **коэффициент пиковой нагрузки x7** от средней нагрузки.
2.  Распределение активности в течение дня: предположим, что основная активность (90%) происходит в течение 16 часов (исключая ночные часы в каждом регионе).
3.  Профиль пользователя: пользователь проводит в приложении 15.65 минут (~940 секунд) в день и генерирует 4 минуты аудио.
4.  Состав урока: урок — это не один запрос, а сессия из множества действий. Оценим, что в среднем за свою сессию пользователь совершает ~50 различных запросов (получить задание, отправить ответ, получить следующее, отправить аудио, обновить прогресс и т.д.). Это ключевое допущение для перехода от времени к RPS.
5.  Трафик: большинство запросов — легкие, JSON-ответы (~1-2 КБ). Но аудио-запросы на загрузку — тяжелые (~2.4 МБ на минуту аудио).
