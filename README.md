...
| Сессии                             | Нет                              | Нет                                                        | По session_id                | Redis           | RDB снепшоты + AOF лог, ежечасно + непрерывно                |
...
| Поиск                              | Нет                              | search_vector, entity_type, language_code                  | По индексам                  | Elasticsearch   | Snapshot, ежедневно                                          |
| Метрики                            | Нет                              | Нет                                                        | Автоматическое               | VictoriaMetrics | Snapshot, ежедневно                                          |
| Логи                               | Нет                              | Нет                                                        | По дате                      | Elasticsearch   | Snapshot, ежедневно                                          |
...
ClickHouse используется только для таблицы "Попытки упражнений", которая шардируется по дате создания (toYYYYMM(attempted_at)). Остальные данные, включая прогресс пользователей, хранятся в PostgreSQL.

MinIO настраивается с Erasure Coding. Медиафайлы распределяются по дате создания автоматически.

Elasticsearch шардирует поисковые индексы по entity_type, а логи - по дате (daily indices). VictoriaMetrics автоматически распределяет метрики по хэшу имени.

Все системы используют встроенные механизмы репликации: PostgreSQL - streaming replication, Redis - Sentinel, ClickHouse - ReplicatedMergeTree, MinIO - Erasure Coding, Elasticsearch - межшардовая репликация.

### Балансировка запросов / мультиплексирование подключений

Для балансировки нагрузки в PostgreSQL используется PgBouncer - легковесный пулер соединений.
Он создает пул из 200 постоянных соединений к базе данных, которые переиспользуются между клиентами.
Это значительно снижает нагрузку на сервер БД, так как исключает постоянное создание и закрытие соединений.
Запросы от приложения распределяются по доступным соединениям в пуле.
...
Используются пулы соединений по 100 подключений на инстанс, что позволяет эффективно переиспользовать установленные соединения и снизить задержки.
...
ClickHouse поддерживает мультиплексирование запросов через HTTP-интерфейс,
что позволяет эффективно обрабатывать множество одновременных запросов.

Для балансировки нагрузки в MinIO используется NGINX с алгоритмом round-robin,
который равномерно распределяет запросы между узлами MinIO.
Поддерживаются keep-alive соединения для эффективной передачи файлов,
что уменьшает задержки при частых операциях с объектами.
