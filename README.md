...
| Напоминания                         | Нет                              | Нет                                                        | Нет                          | BadgerDB         | Полный, ежедневно                                            |

| СУБД            | Таблицы/данные                                                                                                                                                                                                                                                                                                                                                                                                                             | Объём данных   | Шардирование                                      | Шардов   | Реплики   |
|-----------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|---------------------------------------------------|----------|-----------|
| PostgreSQL      | Пользователи, Языки, Курсы, Счетчики курсов, Разделы курса, Модули, Уроки, Типы упражнений, Упражнения уроков, Упражнения с множественным выбором, Упражнения на сопоставление слов, Упражнения на аудирование, Упражнения на говорение, Упражнения на правописание, Буква, Прогресс пользователя, Аудиоданные, Настройки уведомлений, Подписки, Посты пользователей, Комментарии, Достижения, Достижения пользователей, Ежедневные цели   | ~230 TB        | Горизонтальное                                    | 140      | 3         |
| Redis           | Сессии                                                                                                                                                                                                                                                                                                                                                                                                                                     | 50 GB          | По session_id                                     | 5        | 2         |
| MinIO           | Файлы изображений, Аудио файлы                                                                                                                                                                                                                                                                                                                                                                                                             | ~235 TB        | По дате создания                                  | 12       | 3         |
| ClickHouse      | Попытки упражнений                                                                                                                                                                                                                                                                                                                                                                                                                         | 5 TB           | Горизонтально по created_at                       | 50       | 3         |
| Elasticsearch   | Поиск, Логи                                                                                                                                                                                                                                                                                                                                                                                                                                | 30 TB          | По индексам/по дате                               | 20       | 2         |
| VictoriaMetrics | Метрики                                                                                                                                                                                                                                                                                                                                                                                                                                    | 5 TB           | Автоматическое                                    | 10       | 2         |
| BadgerDB        | Напоминания                                                                                                                                                                                                                                                                                                                                                                                                                                | 20 GB          | Нет                                               | 1        | 2         |
...
MinIO настраивается с Erasure Coding. Медиафайлы распределяются по дате создания автоматически.
...
В образовательных платформах используется для проверки упражнений, 
оценки произношения.
#### Алгоритм
...
| BadgerDB     | Локальное хранилище для напоминаний                                           | Встроенный KV-store на диске, высокая производительность для небольших ключ-значение данных, поддержка ACID               |
...
| Prometheus                                    | Сбор метрик инфраструктуры и приложений   | Высокая производительность, поддержка alerting, интеграция с Grafana                   |
| VictoriaMetrics                               | Хранилище временных рядов метрик          | Эффективное хранение больших объёмов метрик, сжатие, масштабируемость                  |
| Grafana                                       | Визуализация метрик                       | Интерактивные дашборды, поддержка множества источников данных                          |
| ELK Stack (Elasticsearch, Logstash, Kibana)   | Логирование и аналитика логов             | Централизованное хранение логов, мощные возможности поиска и визуализации              |

## 9. Обеспечение надежности 

| Компонент системы                    | Способы резервирования / отказоустойчивости                            | Суть метода                                                                                    |
|--------------------------------------|------------------------------------------------------------------------| ---------------------------------------------------------------------------------------------- |
| PostgreSQL                           | Master + 2 реплики, Patroni для управления кластером, WAL + снепшоты   | Репликация данных, автоматическое переключение на реплику при падении мастера, резервные копии |
| Redis                                | Redis Sentinel, Redis Cluster, репликация, RDB + AOF, failover         | Мониторинг состояния, автоматическое переключение, сохранение данных на диске и в памяти       |
| ClickHouse                           | Shards + репликация, Distributed Tables                                | Данные распределяются по шардам, дублируются для отказоустойчивости                            |
| MinIO                                | Distributed mode, Erasure Coding, replication, versioning              | Восстановление избыточных данных, копирование между узлами, хранение версий объектов           |
| Elasticsearch                        | Шардинг, репликация, discovery nodes                                   | Балансировка нагрузки и автоматическое определение доступных узлов                             |
| VictoriaMetrics                      | vmselect распределяет запросы, репликация, автоматическое шардирование | Отказоустойчивое хранение метрик, балансировка нагрузки                                        |
| BadgerDB                             | Локальное хранение + реплики                                           | Быстрое чтение/запись ключ-значение с резервными копиями                                       |
| Микросервисы                         | Kubernetes ReplicaSet, HPA, Pod Disruption Budget, graceful shutdown   | Автоматическое масштабирование, перезапуск упавших контейнеров, корректное завершение запросов |
| L7 балансировка (NGINX, API Gateway) | Active-Active кластер, Anycast, health-check, rate limiting, fallback  | Балансировка нагрузки, отказоустойчивость, защита от перегрузок                                |
| Дата-центры                          | Multi-AZ, резервные сети, питание, охлаждение                          | Снижение рисков отказа оборудования и инфраструктуры                                           |

### Graceful Degradation Scenarios
#### Lesson Execution - Primary Degradation
В нормальном режиме Lesson Execution берет сет упражнений из Course & Content, 
получает историю попыток и ошибок из History, отправляет эти данные через Kafka в HLR сервис для персонализации, 
получает обратно персонализированный набор упражнений и собирает финальный урок для пользователя. 
При падении HLR сервиса система переходит в режим graceful degradation: 
используются кэшированные данные об уровне пользователя из PostgreSQL, 
берутся стандартные упражнения соответствующего уровня сложности без персонализации, 
все данные о запросах сохраняются в Graceful Degradation Database для последующей пересинхронизации. 
Когда HLR сервис восстанавливается, система автоматически пересчитывает все отложенные персонализации и обновляет кэши.
#### Answer Processing Degradation
При нормальной работе когда пользователь дает неверный ответ, Lesson Execution отправляет вопрос и ответ пользователя через Kafka в Transformer сервис, 
который возвращает развернутое объяснение правильного ответа, после чего Lesson Execution показывает это объяснение пользователю. 
Если Transformer сервис недоступен, система использует правильные ответы из PostgreSQL, 
сохраняет сложные случаи в очередь для последующей обработки, а для распространенных ошибок выдает стандартные объяснения из кэша.
#### Speech Processing Degradation
Для упражнений на аудирование нормальный workflow предполагает отправку аудио ответа пользователя через Kafka в Speech Processing сервис, 
который возвращает транскрипт, после чего транскрипт проверяется на правильность. 
При недоступности Speech Processing система предлагает пользователю альтернативный вариант упражнения с текстовым вводом.
#### Notification System Degradation
Регулярные уведомления работают через Regular Notification сервис, 
который по таймеру берет сообщения из BadgerDB, 
проверяет настройки уведомлений пользователя из PostgreSQL с кэшированием в Redis, 
и отправляет уведомления через Push и Email Delivery сервисы. 
При падении основного Notification сервиса система использует шаблонные уведомления без глубокой персонализации, 
сохраняет информацию о предназначенных уведомлениях в очередь восстановления, обеспечивая базовый функционал напоминаний. 
Все микросервисы пишут события для уведомлений в Kafka, откуда они попадают в Events сервис и затем в BadgerDB для последующей обработки.

## 10. Схема проекта
![image](./images/hw.drawio.png)

## 11. Список серверов
### Конфигурация физических серверов

Application Servers (микросервисы):
- Модель: Dell PowerEdge R7625
- CPU: 2× AMD EPYC 9354P (32 core/64 thread, 3.25GHz) = 64 cores
- RAM: 8× 32GB DDR5 4800MHz = 256GB
- Сеть: 2× 25Gb SFP28 + 1× 1Gb management
- Диски: 2× 960GB NVMe SSD (OS) + 4× 3.84TB NVMe SSD (data)
- Цена: $16,000 за сервер

PostgreSQL Servers (шардированная БД):
- Модель: Supermicro AS-2125HS-TNR
- CPU: 2× AMD EPYC 9454P (48 core/96 thread, 2.75GHz) = 96 cores
- RAM: 16× 64GB DDR5 4800MHz = 1024GB
- Сеть: 2× 100Gb QSFP56 + 1× 1Gb management
- Диски: 2× 1.92TB NVMe SSD (OS) + 12× 15.36TB NVMe SSD (data)
- Цена: $35,000 за сервер

Redis Servers (шардированный кеш):
- Модель: Dell PowerEdge R7625
- CPU: 2× AMD EPYC 9334 (32 core/64 thread, 2.7GHz) = 64 cores
- RAM: 16× 32GB DDR5 4800MHz = 512GB
- Сеть: 2× 25Gb SFP28 + 1× 1Gb management
- Диски: 2× 1.92TB NVMe SSD (OS) + 4× 3.84TB NVMe SSD (data)
- Цена: $22,000 за сервер

MinIO Storage Servers (объектное хранилище):
- Модель: Dell PowerEdge R7625
- CPU: 2× AMD EPYC 9334 (32 core/64 thread, 2.7GHz) = 64 cores
- RAM: 8× 16GB DDR5 4800MHz = 128GB
- Сеть: 2× 100Gb QSFP56
- Диски: 2× 960GB NVMe SSD (OS) + 24× 15.36TB SAS HDD (data)
- Цена: $28,000 за сервер

Analytics Servers (ClickHouse, Elasticsearch, VictoriaMetrics):
- Модель: Supermicro AS-2125HS-TNR
- CPU: 2× AMD EPYC 9354P (32 core/64 thread, 3.25GHz) = 64 cores
- RAM: 16× 32GB DDR5 4800MHz = 512GB
- Сеть: 2× 100Gb QSFP56 + 1× 1Gb management
- Диски: 2× 1.92TB NVMe SSD (OS) + 8× 7.68TB NVMe SSD (data)
- Цена: $28,000 за сервер

### Расчёт для микросервисов

| Микросервис    | Пиковый RPS | CPU | RAM | Сетевой трафик (Гбит/с) | Кол-во серверов | Конфигурация сервера |
|----------------|-------------|-----|-----|-------------------------|-----------------|---------------------|
| Auth           | 579         | 2   | 1   | 0.9                     | 1               | Application Server  |
| User           | 579         | 2   | 1   | 0.9                     | 1               | Application Server  |
| Course & Content | 6,072     | 13  | 12  | 55.96                   | 5               | Application Server  |
| Lesson Execution | 6,072     | 13  | 12  | 55.96                   | 5               | Application Server  |
| History        | 289         | 1   | 1   | 56.52                   | 5               | Application Server  |
| Progress Tracking | 289       | 1   | 1   | 56.52                   | 5               | Application Server  |
| Social         | 868         | 2   | 2   | 62.4                    | 5               | Application Server  |
| Regular Notification | 290    | 1   | 1   | 1.12                    | 1               | Application Server  |
| Events         | 290         | 1   | 1   | 1.12                    | 1               | Application Server  |
| Notification Search | 290     | 1   | 1   | 1.12                    | 1               | Application Server  |
| Payment        | 116         | 1   | 1   | 22.58                   | 2               | Application Server  |
| ИТОГО          | 15,834      | 38  | 34  | 315.1                   | 32              |                     |

### Расчёт для баз данных 

| СУБД            | Объём данных | Шардов | Реплик | Всего серверов | CPU cores | RAM GB | Кол-во серверов | Конфигурация сервера |
|-----------------|--------------|--------|--------|----------------|-----------|---------|-----------------|---------------------|
| PostgreSQL      | 230 TB       | 140    | 3      | 420            | 2,940     | 430,080 | 35              | PostgreSQL Server   |
| Redis           | 50 GB        | 5      | 2      | 10             | 64        | 5,120   | 2               | Redis Server        |
| MinIO           | 235 TB       | 12     | 3      | 36             | 144       | 4,608   | 9               | Storage Server      |
| ClickHouse      | 5 TB         | 50     | 3      | 150            | 450       | 76,800  | 12              | Analytics Server    |
| Elasticsearch   | 30 TB        | 20     | 2      | 40             | 520       | 20,480  | 8               | Analytics Server    |
| VictoriaMetrics | 5 TB         | 10     | 2      | 20             | 20        | 10,240  | 4               | Analytics Server    |
| BadgerDB        | 20 GB        | 1      | 2      | 2              | 4         | 128     | 1               | Application Server  |
| ИТОГО БД        | 575 TB       | 238    | 17     | 678            | 4,142     | 547,456 | 71              |                     |

### Итоги по серверам и стоимость

| Компонент       | Серверы | Конфигурация сервера | Всего CPU | Всего RAM | Стоимость за сервер | Общая стоимость |
|-----------------|---------|---------------------|-----------|-----------|---------------------|-----------------|
| Микросервисы    | 32      | Application Server  | 2,048 cores | 8,192 GB | $16,000 | $512,000 |
| PostgreSQL      | 35      | PostgreSQL Server   | 3,360 cores | 35,840 GB | $35,000 | $1,225,000 |
| Redis           | 2       | Redis Server        | 128 cores   | 1,024 GB  | $22,000 | $44,000  |
| MinIO           | 9       | Storage Server      | 576 cores   | 1,152 GB  | $28,000 | $252,000 |
| ClickHouse      | 12      | Analytics Server    | 768 cores   | 6,144 GB  | $28,000 | $336,000 |
| Elasticsearch   | 8       | Analytics Server    | 512 cores   | 4,096 GB  | $28,000 | $224,000 |
| VictoriaMetrics | 4       | Analytics Server    | 256 cores   | 2,048 GB  | $28,000 | $112,000 |
| BadgerDB        | 1       | Application Server  | 64 cores    | 256 GB    | $16,000 | $16,000  |
| Общее           | 103     |                     | 7,712 cores | 58,752 GB |         | $2,721,000 |

### Конфигурация по Worker Nodes

| Сервис                 | CPU/r (cores) | CPU/l (cores) | RAM/r (GB) | RAM/l (GB) | Кол-во реплик | Серверы |
|------------------------|---------------|---------------|------------|------------|---------------|---------|
| Auth                   | 0.67          | 1.34          | 0.33       | 0.66       | 3             | 1× Application |
| User                   | 0.67          | 1.34          | 0.33       | 0.66       | 3             | 1× Application |
| Course & Content       | 4.33          | 8.66          | 4          | 8          | 3             | 5× Application |
| Lesson Execution       | 4.33          | 8.66          | 4          | 8          | 3             | 5× Application |
| History                | 0.33          | 0.66          | 0.33       | 0.66       | 3             | 5× Application |
| Progress Tracking      | 0.33          | 0.66          | 0.33       | 0.66       | 3             | 5× Application |
| Social                 | 0.67          | 1.34          | 0.67       | 1.34       | 3             | 5× Application |
| Regular Notification   | 0.33          | 0.66          | 0.33       | 0.66       | 3             | 1× Application |
| Events                 | 0.33          | 0.66          | 0.33       | 0.66       | 3             | 1× Application |
| Notification Search    | 0.33          | 0.66          | 0.33       | 0.66       | 3             | 1× Application |
| Payment                | 0.33          | 0.66          | 0.33       | 0.66       | 3             | 2× Application |
| Prometheus             | 0.67          | 1.34          | 1.33       | 2.66       | 3             | 1× Analytics |
| Grafana                | 0.17          | 0.34          | 0.33       | 0.66       | 3             | 1× Application |
