...
- [**9. Обеспечение надежности**](#7-обеспечение-надежности)
- [**10. Схема проекта**](#7-схема-проекта)
- [**11. Список серверов**](#7-список-серверов)
...
### Синхронизация и асинхронные паттерны

- Saga с оркестратором управляет последовательностью действий между микросервисами (Lessons → Images → Answers → Mistakes).
- Kafka обеспечивает очередь сообщений и синхронизацию событий между сервисами.
- CQRS + аналитика: записи статистики идут в ClickHouse через RabbitMQ, чтение отделено для масштабирования нагрузки.

### Observability: метрики и логирование

| Компонент   | Подход                                                                                                                               |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| Логирование | Access / Error / Work, централизованный сбор в Elasticsearch, семплирование по request_id                                            |
| Трейсинг    | Сквозное отслеживание request_id для анализа цепочек вызовов                                                                         |
| Метрики     | Системные (CPU, RAM, диск), бизнесовые (RPS, latency, error rate), сбор Prometheus + визуализация Grafana, алерты через Alertmanager |
| Status Page | Публикация доступности сервисов для пользователей                                                                                    |
...
| Микросервис    | Пиковый RPS      | CPU   | RAM   | Сетевой трафик (Гбит/с)   | Кол-во серверов (64c/64GB)  |
|----------------|------------------|-------|-------|---------------------------|-----------------------------|
| Auth           | 579 ×2=1,158     | 2     | 1     | 0.9×2=1.8                 | 1                           |
| Lessons        | 6,072 ×2=12,144  | 13    | 12    | 55.96×2=111.92            | 5                           |
| Progress       | 289 ×2=578       | 1     | 1     | 56.52×2=113.04            | 5                           |
| Goals          | 289 ×2=578       | 1     | 1     | 2.26×2=4.52               | 1                           |
| Notifications  | 145 ×2=290       | 1     | 1     | 1.12×2=2.24               | 1                           |
| Social         | 434 ×2=868       | 1     | 1     | 62.4×2=124.8              | 5                           |
| Support        | 58 ×2=116        | 1     | 0.5   | 22.58×2=45.16             | 2                           |
| Authoring      | 29 ×2=58         | 1     | 0.5   | 22.66×2=45.32             | 2                           |
| ИТОГО          | 10,326           | 21    | 17    | 448.8                     | 22                          |

Примечания:

* CPU: суммируем `ceil(пиковый RPS / 500)` по эндпоинтам сервиса.
* Сетевой трафик: суммируем Гбит/с всех эндпоинтов и умножаем на коэффициент пика ×2.
* Количество серверов: рассчитывается по CPU и RAM с учётом конфигурации 64c/64GB и сетевых ограничений (≤25 Gbit/s на сервер).
* Все микросервисы можно разместить в Kubernetes кластере bare-metal с соответствующим распределением по нодам.

### Расчёт для баз данных

| СУБД            | Объём данных | RPS суммарный | Среднее время запроса | CPU cores (≈) | RAM GB (≈)  | Кол-во серверов (64c/64GB) |
|-----------------|--------------|---------------|-----------------------|---------------|-------------|----------------------------|
| PostgreSQL      | 230 TB       | ~4,150        | 0.015 с               | 125           | 11,500      | 180                        |
| Redis           | 50 GB        | 289           | 0.0015 с              | 1             | 55          | 1                          |
| MinIO           | 235 TB       | 2             | 0.007 с               | 2             | 4,700       | 74                         |
| ClickHouse      | 5 TB         | 144           | 0.007 с               | 2             | 500         | 8                          |
| Elasticsearch   | 30 TB        | 144           | 0.03 с                | 9             | 3,000       | 47                         |
| VictoriaMetrics | 5 TB         | 72            | 0.003 с               | 1             | 500         | 8                          |
| BadgerDB        | 20 GB        | 72            | 0.0015 с              | 1             | 22          | 1                          |

Примечания:

* CPU-расчёт: `CPU cores ≈ RPS × среднее время запроса × 2 (коэффициент пикового запаса)`.
* RAM-расчёт: ориентирован на общий объём данных × коэффициент (для кешей и реплик).
* Кол-во серверов: `max(CPU/64, RAM/64)`, округляем вверх.

### Итоги по серверам

| Компонент       | Серверы  | Всего CPU (cores) | Всего RAM (GB)   | Стоимость оборудования ($) |
|-----------------|----------|-------------------|------------------|----------------------------|
| Микросервисы    | 10       | 21                | 18               | 160,000                    |
| PostgreSQL      | 180      | 11,520            | 11,500           | 3,600,000                  |
| Redis           | 1        | 1                 | 55               | 20,000                     |
| MinIO           | 74       | 148               | 4,700            | 1,480,000                  |
| ClickHouse      | 8        | 16                | 500              | 160,000                    |
| Elasticsearch   | 47       | 423               | 3,000            | 940,000                    |
| VictoriaMetrics | 8        | 8                 | 500              | 160,000                    |
| BadgerDB        | 1        | 1                 | 22               | 20,000                     |
| Итого БД        | 319      | 12,117            | 20,277           | 6,380,000                  |
| Общее           | 329      | 12,138            | 20,294           | 6,540,000                  |

### Конфигурация по Worker Nodes

| Сервис        | CPU/r (cores)  | CPU/l (cores) | RAM/r (GB)     | RAM/l (GB) | Кол-во реплик | Обоснование                                 |
|---------------|----------------|---------------|----------------|------------|---------------|---------------------------------------------|
| Auth          | 2 / 3 ≈ 0.67   | 1.34          | 1 / 3 ≈ 0.33   | 0.66       | 3             | Requests на экземпляр, limit ×2 для пиков   |
| Lessons       | 13 / 3 ≈ 4.33  | 8.66          | 12 / 3 = 4     | 8          | 3             | Requests на экземпляр, limit ×2             |
| Progress      | 1 / 3 ≈ 0.33   | 0.66          | 1 / 3 ≈ 0.33   | 0.66       | 3             | Лёгкий сервис, limit ×2                     |
| Goals         | 1 / 3 ≈ 0.33   | 0.66          | 1 / 3 ≈ 0.33   | 0.66       | 3             | Лёгкий сервис, limit ×2                     |
| Notifications | 1 / 3 ≈ 0.33   | 0.66          | 1 / 3 ≈ 0.33   | 0.66       | 3             | Лёгкий сервис, limit ×2                     |
| Social        | 1 / 3 ≈ 0.33   | 0.66          | 1 / 3 ≈ 0.33   | 0.66       | 3             | Лёгкий сервис, limit ×2                     |
| Support       | 1 / 3 ≈ 0.33   | 0.66          | 0.5 / 3 ≈0.17  | 0.34       | 3             | Лёгкий сервис, limit ×2                     |
| Authoring     | 1 / 3 ≈ 0.33   | 0.66          | 0.5 / 3 ≈0.17  | 0.34       | 3             | Лёгкий сервис, limit ×2                     |
| Prometheus    | 2 / 3 ≈ 0.67   | 1.34          | 4 / 3 ≈ 1.33   | 2.66       | 3             | Мониторинг метрик, limit ×2                 |
| Grafana       | 0.5 / 3 ≈0.17  | 0.34          | 1 / 3 ≈0.33    | 0.66       | 3             | Визуализация метрик, limit ×2               |

Общие показатели по Worker Nodes:
* Всего нод: 341
* Общая CPU: 12,138 cores
* Общая RAM: 20,295 GB (~20,3 TB)
* Использование: CPU ~36%, RAM ~76%
* Запас для масштабирования: CPU ×2–3, RAM ×1.3–2 (для пиков и добавления реплик)
