...
### Расчет объемов данных и шардирование

| СУБД            | Таблицы/данные                                                                                    | Текущий объём  | Объём на 3 года | Шардирование               | Шардов | Реплик | Обоснование                                                                                   |
|-----------------|---------------------------------------------------------------------------------------------------|----------------|-----------------|----------------------------|--------|--------|-----------------------------------------------------------------------------------------------|
| PostgreSQL      | Пользователи, Языки, Курсы, Прогресс, Настройки, Подписки, Посты, Комментарии, Достижения, цели   | 3.5 ТБ         | 10.5 ТБ         | Горизонтальное по user_id  | 10     | 3      | 10.5 ТБ ÷ 1 ТБ/шард = 10 шардов                                                               |
| Redis           | Сессии                                                                                            | 50 GB          | 150 GB          | По session_id              | 5      | 2      | 12.5 млн DAU × 1KB/сессия × 3 дня хранения = ~37.5 GB × 3 года = 150 GB                       |
| MinIO           | Файлы изображений, Аудио файлы                                                                    | 225.2 ТБ       | 675.6 ТБ        | По дате создания           | 12     | 3      | 225.2 ТБ аудио × 3 года = 675.6 ТБ                                                            |
| ClickHouse      | Попытки упражнений                                                                                | 5 ТБ           | 15 ТБ           | По created_at              | 15     | 3      | 446 млн действий/день × 100 байт × 1095 дней = ~48 TB ÷ 3 = 15 TB                             |
| Elasticsearch   | Поиск, Логи                                                                                       | 30 ТБ          | 90 ТБ           | По индексам/по дате        | 10     | 2      | 30 ТБ × 3 года = 90 ТБ                                                                        |
| VictoriaMetrics | Метрики                                                                                           | 5 ТБ           | 15 ТБ           | Автоматическое             | 5      | 2      | 5 ТБ × 3 года = 15 ТБ                                                                         |
| BadgerDB        | Напоминания                                                                                       | 20 GB          | 60 GB           | Нет                        | 1      | 2      | Объем небольшой                                                                               |

Redis развертывается в конфигурации Sentinel. Сессии шардируются по session_id автоматически средствами Redis.
...
| Название                        | Роль                            | CPU       | RAM     | Цена    |
|---------------------------------|---------------------------------|-----------|---------|---------|
| Dell EPYC 9354P Server          | Микросервисы                    | 64 cores  | 256GB   | $16,000 |
| Supermicro EPYC 9454P DB Server | PostgreSQL мастер               | 96 cores  | 1TB     | $35,000 |
| Supermicro EPYC 9454P Replica   | PostgreSQL реплика              | 96 cores  | 512GB   | $28,000 |
| Dell EPYC 9334 Cache Server     | Redis                           | 64 cores  | 512GB   | $22,000 |
| Dell EPYC 9334 Storage Server   | MinIO                           | 64 cores  | 128GB   | $28,000 |
| Supermicro EPYC 9354P Analytics | ClickHouse, ES, VictoriaMetrics | 64 cores  | 512GB   | $28,000 |

### Расчёт для микросервисов

Допущения:
- Каждое ядро обрабатывает 400 RPS
- Пиковый RPS = 10,331.8 (5,165.9 × 2)
- Запас 20% на непредвиденную нагрузку

| Микросервис            | Пиковый RPS  | CPU cores   | CPU +20%   | RAM    | RAM +20%   | Реплик   | Серверов   |
|------------------------|--------------|-------------|------------|--------|------------|----------|------------|
| Auth                   | 579          | 1.45        | 1.7        | 2GB    | 2.4GB      | 2        | 1          |
| User                   | 289          | 0.72        | 0.9        | 2GB    | 2.4GB      | 2        | 1          |
| Course & Content       | 87           | 0.22        | 0.3        | 15GB   | 18GB       | 3        | 1          |
| Lesson Execution       | 8,102        | 20.26       | 24.3       | 15GB   | 18GB       | 3        | 1          |
| History                | 289          | 0.72        | 0.9        | 2GB    | 2.4GB      | 2        | 1          |
| Progress Tracking      | 289          | 0.72        | 0.9        | 2GB    | 2.4GB      | 2        | 1          |
| Social                 | 810          | 2.03        | 2.4        | 4GB    | 4.8GB      | 3        | 1          |
| Regular Notification   | 145          | 0.36        | 0.4        | 2GB    | 2.4GB      | 2        | 1          |
| Events                 | 300          | 0.75        | 0.9        | 2GB    | 2.4GB      | 2        | 1          |
| Notification Search    | 100          | 0.25        | 0.3        | 2GB    | 2.4GB      | 2        | 1          |
| Payment                | 50           | 0.13        | 0.2        | 2GB    | 2.4GB      | 2        | 1          |
| ИТОГО                  | 10,940       | 27.35       | 32.8       | 50GB   | 60GB       | 25       | 10         |


### Расчёт для баз данных
| СУБД            | Объём на 3 года | Шардов | Реплик | Всего узлов | Физ. серверов | Обоснование расчета                                                                                                            |
|-----------------|-----------------|--------|--------|-------------|---------------|--------------------------------------------------------------------------------------------------------------------------------|
| PostgreSQL      | 10.5 ТБ         | 10     | 3      | 30          | 10            | 30 узлов ÷ 3 узла/сервер = 10 серверов (1 шард со всеми репликами на одном сервере для уменьшения сетевых задержек)            |
| Redis           | 150 GB          | 5      | 2      | 10          | 2             | 10 узлов ÷ 5 узлов/сервер = 2 сервера (каждый сервер: 512GB RAM ÷ 150GB/5 шардов = 17 узлов, но ограничено 5 для балансировки) |
| MinIO           | 675.6 ТБ        | 12     | 3      | 36          | 9             | 36 узлов ÷ 4 узла/сервер = 9 серверов                                                                                          |
| ClickHouse      | 15 ТБ           | 15     | 3      | 45          | 9             | 45 узлов ÷ 5 узлов/сервер = 9 серверов                                                                                         |
| Elasticsearch   | 90 ТБ           | 10     | 2      | 20          | 4             | 20 узлов ÷ 5 узлов/сервер = 4 сервера                                                                                          |
| VictoriaMetrics | 15 ТБ           | 5      | 2      | 10          | 2             | 10 узлов ÷ 5 узлов/сервер = 2 сервера                                                                                          |
| BadgerDB        | 60 GB           | 1      | 2      | 2           | 1             | 2 узла ÷ 2 узла/сервер = 1 сервер                                                                                              |
| ИТОГО БД        | 806.2 ТБ        | 58     | 17     | 153         | 37            |                                                                                                                                |

### Итоги по серверам и стоимость
| Компонент          | Серверов | Тип сервера                          | Цена за сервер | Итого стоимость |
|--------------------|----------|--------------------------------------|----------------|-----------------|
| Микросервисы       | 10       | Dell EPYC 9354P Server               | $16,000        | $160,000        |
| PostgreSQL         | 10       | Supermicro EPYC 9454P DB Server      | $35,000        | $350,000        |
| Redis              | 2        | Dell EPYC 9334 Cache Server          | $22,000        | $44,000         |
| MinIO              | 9        | Dell EPYC 9334 Storage Server        | $28,000        | $252,000        |
| ClickHouse         | 9        | Supermicro EPYC 9354P Analytics      | $28,000        | $252,000        |
| Elasticsearch      | 4        | Supermicro EPYC 9354P Analytics      | $28,000        | $112,000        |
| VictoriaMetrics    | 2        | Supermicro EPYC 9354P Analytics      | $28,000        | $56,000         |
| BadgerDB           | 1        | Dell EPYC 9354P Server               | $16,000        | $16,000         |
| ИТОГО              | 47       |                                      |                | $1,242,000      |

### Конфигурация Kubernetes
- Всего нод: 10 серверов
- На ноду выделяем: 52 ядра (81%), 220GB RAM (86%) для приложений
- Остальные ресурсы для системных компонентов (kubelet, DNS, сетевые плагины)
- Всего для приложений в кластере: 10 × 52 = 520 ядер, 10 × 220 = 2,200GB RAM
- Требуется для микросервисов: 32.8 ядер CPU, 60GB RAM
- Используется от общей емкости: CPU - 6.3%, RAM - 2.7%
- Запас для масштабирования: CPU - 93.7%, RAM - 97.3%

Распределение подов по нодам:

| Сервис               | Реплик | CPU/реплика | RAM/реплика | Размещение          |
|----------------------|--------|-------------|-------------|---------------------|
| Auth                 | 2      | 0.85 cores  | 1.2GB       | На разных нодах     |
| User                 | 2      | 0.45 cores  | 1.2GB       | На разных нодах     |
| Course & Content     | 3      | 0.1 cores   | 6GB         | На 3 разных нодах   |
| Lesson Execution     | 3      | 8.1 cores   | 6GB         | На 3 разных нодах   |
| History              | 2      | 0.45 cores  | 1.2GB       | На разных нодах     |
| Progress Tracking    | 2      | 0.45 cores  | 1.2GB       | На разных нодах     |
| Social               | 3      | 0.8 cores   | 1.6GB       | На 3 разных нодах   |
| Regular Notification | 2      | 0.2 cores   | 1.2GB       | На разных нодах     |
| Events               | 2      | 0.45 cores  | 1.2GB       | На разных нодах     |
| Notification Search  | 2      | 0.15 cores  | 1.2GB       | На разных нодах     |
| Payment              | 2      | 0.1 cores   | 1.2GB       | На разных нодах     |
